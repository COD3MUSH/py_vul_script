...
then
  ...
  echo "Check Result : UnSafe" >> $CREATE_FILE 2>&1
...
then
  echo "Check Result : There are no such files or directories" >> $CREATE_FILE 2>&1
else
  echo "Check Result : UnSafe" >> $CREATE_FILE 2>&1
  ls -alL /etc/xinetd.conf >> $CREATE_FILE 2>&1
fi
...
There are no such files or directories" >> $CREATE_FILE 2>&1
else
  echo "Check Result : UnSafe" >> $CREATE_FILE 2>&1
  ls -alL /etc/rsyslog.conf >> $CREATE_FILE 2>&1
fi
